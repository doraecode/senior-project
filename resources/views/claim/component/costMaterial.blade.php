...
<nav class="nav nav-pills nav-fill">
  ...
  <a class="nav-item nav-link active" href="http://localhost:8000/register/claims/{{$customer->id}}/cost-material">ค่าอะไหล่</a>
...
<br><br>
<form  method="POST" action="/claims/costMaterial/post">
      {{ csrf_field() }}
    ...
    <tbody >
      <?php foreach ($costMaterials as $costMaterial ): ?>
        <tr>
      <td>{{$costMaterial->number}}</td>
        <td>{{$costMaterial->name}}</td>
        <td>{{$costMaterial->damageLevel}}</td>
        <td>{{$costMaterial->price}}</td>
        <td>{{$costMaterial->priceOffer}}</td>
        <td>{{$costMaterial->priceOk}}</td>
        <td>{{$costMaterial->junk}}</td>
      ...
      <option>เปลี่ยน</option>
          ...
          <td width="150px"><select class="form-control" name="Junk" id="Junk" >
      <option></option>
      <option>คืนซาก</option>
      <option>ไม่คืนซาก</option>
